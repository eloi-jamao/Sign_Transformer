...
class SNLT_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        img_fold = os.path.join(self.img_dir, self.samples[idx][0],"")[:-1]
    ...
    def make_clips(self, image_folder, long, window, max_len = 60):
        ...
        if sequence[-1].shape[2] < long:
            ...
            sequenceB = torch.cat((sequence[-1],torch.zeros((1, 3,long-sequence[-1].shape[2],112,112)).to(self.device)),dim=2,)
            #print('B',sequenceB.shape)
            sequence = torch.cat((sequenceA,sequenceB), dim = 0)
        else:
            sequence = torch.cat(sequence,dim=0).to(self.device)
        #print(sequence.shape)
        sequence = torch.cat((sequence,torch.zeros((max_len-sequence.size()[0],3,6,112,112)).to(self.device)), dim = 0)
        return sequence
...
if __name__ == '__main__':
    ...
    frames = '../Sign_Transformer1/data/PHOENIX-2014-T-release-v3/PHOENIX-2014-T/features/fullFrame-210x260px/'
    dataset = SNLT_Dataset(split = 'train', dev = 'cpu', frames_path = frames, csv_path = "data/annotations/", gloss = False)
    #train_loader = DataLoader(dataset, batch_size = 4, shuffle = False)


    it = iter(Custom_iterator(dataset, 6, num_workers = 1).pool_iterate())
    start = time.time()
    batch = next(it)
